...
class Weapons(Item):
    def __init__(self, name, eat, attack, damage, shrink, splash, spread, description):
        self.eat = eat
        self.attack = attack
        self.damage = damage
        self.shrink = shrink
...
class Archery(Item):
    def __init__(self, name, attack, damage):
        self.name = name
        super(Archery, self).__init__(name, '')
        self.attack = attack
        self.damage = damage
...
class Bow(Archery):
    def __init__(self):
        super(Bow, self).__init__('Bow', 40, 1)


class Sword(Weapons):
    def __init__(self):
        super(Sword, self).__init__('Sword', False, 100, 100, False, False, False, 'A sword to keep you safe')

class Knife(Weapons):
    def __init__(self):
        super(Knife, self).__init__('knife', False, 20, 20, False, False, False, 'In the upper right corner you see a '
                                                                                 'pocket knife')
...
class Otherwearables(Item):
    def __init__(self):
        super(Otherwearables, self).__init__('clothes', '')
...
sword = Sword()
toast = Item('toast', 'A piece of toast. Might be good with the avocado you found.')
avocado = Item('avocado', 'A random avocado. Might as well take it.')
avocado2 = Item('avocado', 'Another random avocado, why another avocado')
plums = Item('plums', 'Plums that were bought in Romania are in the room')
mac_n_burger = Item('burger', 'Your favorite burger. Reminds you of home')
half_eaten_sandwich = Item('sandwich', 'A half-eaten sandwich')
knife = Item('knife', 'In the upper right corner, there is a pocket knife')
key = Item('key', 'The key you need to gain the trust of the ducks')
bottle = Item('water bottle', 'random water bottle')
...
bucky = Character('Bucky Barnes', 'Your best friend and would protect you from anything.', '"Would you like some '
                                                                                           ...
                                                                                           'you take some."', '', '',
                  '', [plums])
steve = Character('Steve Rogers', 'Your best friend and would protect you from anything.', '"I can do this all day."',
                  '', '', '')
sam = Character('Sam Wilson', '', '"Don\'t trust the ducks... they\'re dangerous..."', '', '', '')
...
LISTOFCOMMANDS = 'North, South, East, West, Northwest, Southeast, Southwest, Up, Down, \nLook, Quit, Party, Travel, ' \
                 'Inventory, Commands, pick up, drop'
...
KITCHEN = 'You are now in the kitchen.\nThere is four paths: West, North, South, and East. Also you see a very ' \
          'familiar person in the corner.'
...
CAVE_2 = 'You are in another dirt cave. There is two paths to the southwest, southeast and north.'
CAVE_2_2 = 'You re-enter the dirt cave. There is two paths to the southwest, southeast and north.'
...
LARSTORAGE = 'You enter a dim lighted storage room. You see over 3,000 tiny rubber ducks in the room ' \
             '\nsurrounding a the largest rubber duck in the world. You need to pick up the key in the room. \nThen ' \
             'the ducks will not attack you. You can go up the stairs and once you have the key you can go east.'
LARSTORAGE2 = 'You are back. Not quite sure if the duct you yet. Have fun I guess... :('
PARTY = 'You gained the ducks trust. You walk into a party room to celebrate the win ' \
...
COMPOUND = 'WOW! It\'s the Avengers Compound. That\'s where they came from. Go down to head back to the basement. ' \
           '\nPepper Potts is walking around trying to figure out where everyone went.'
...
kitchen = Room('Kitchen', KITCHEN, KITCHEN2, 'hole_2', 's_box', 'bathroom_1', 'bedroom', '', '', '', '', '', '',
               [knife])
hole = Room('Hole', HOLE_1, HOLE_1_2, '', 'cave_1', '', '', '', '', '', '', '', '', [])
cave_1 = Room('Cave', CAVE_1, CAVE_1_2, 'hole', 'deadzone', 'm_box', '', '', '', '', '', '', '', [])
bathroom_1 = Room('Bathroom', BATH_1, BATH_1_2, '', 'storage_room', 'grass_field', 'kitchen', '', '', '', '', '', '',
                  [half_eaten_sandwich])
hole_2 = Room('Hole', HOLE_2, HOLE_2, '', '', '', '', 'cave_2', '', '', '', '', '', [])
cave_2 = Room('Cave', CAVE_2, CAVE_2_2, 'deadzone', '', '', '', '', 'hole_2', 'bedroom', '', '', '', [])
storage_room = Room('Storage Room', STORAGE, STORAGE2, 'bathroom_1', '', '', '', '', '', '', '', '', '', [avocado])
s_box = Room('Box', S_BOX, '', 'kitchen', '', '', 'm_box', '', '', '', '', '', '', [])
grass_field = Room('Grass Field', GRASS, GRASS2, '', 'barn', '', 'bathroom_1', '', '', '', '', '', '', [bottle])
...
deadzone = Room('Trap', DEADZONE, '', '', '', '', '', '', '', '', '', '', '', [])


travel_map = Map('Travel Map')
...
while response not in ['yes', 'no']:
    ...
    if response.lower() == 'yes':
        ...
        print("Welcome. Here is some basic information."
              ...
              "\n---------------------------------------------------------------------------"
              "\nCommands:"
              "\nLook, Quit, Party, Travel, Inventory, Commands, Help, Take, Drop."
              "\n--------------------------------------------------------------------"
              "\nDirections:"
              ...
              "\n-----------------------------------------------------------"
              "\nTake allows you to pick up items."
              "\n------------------------------------"
...
print()
print("To pick up or drop items, type 'pick up' or 'drop'")
print()
print(info)
print('////////////////////////////////////////////////////////////////////////////////////////////////////////////////'
      '///////////////////////////////////////////////////////////////////////')
...
while is_playing:
    ...
    if command == 'party':
        ...
        for stuff in current_node.inventory:
            if item == stuff.name:
                ...
                current_node.inventory.remove(stuff)
    elif command[:4] == 'drop':
        item = command[5:]
    ...
    if current_node == bedroom:
        print(sam.name)
        print(sam.dialogue)
    if command == 'enter pool':
        print(deadzone.description)
        ...
        if response.lower() == 'yes':
            ...
            Mr_Wybe = Character('Mr. Wybe', 'HYDRA Agent, Your Father, and a completely horrible parent.', '', '', '',
                                '', [])
            player = Character('Mister Sir Man', 'Loving kid, smart, and adventurous.', '', '', '', '', [])
            bucky = Character('Bucky Barnes', 'Your best friend and would protect you from anything.',
                              '"Would you like some '
                              'plums. I don\'t mind it '
                              'you take some."', '', '',
                              '', [plums])
            BACKSTORY = 'You are heading over to your Uncle Wiebe\'s house with your dad, Mr. Wybe, and your older ' \
                        'brother, Wiebe "The Duck" Wybe. "How ya doing sis?" said Wiebe. \n"Good. Dad\'s music is ' \
                        'annoying tho." you said. "It is not!" yelled. Mr. Wybe. You pull up to your Uncle\'s house ' \
                        'and enter in. A hour later your dad and uncle get into a fight. \nYour dad left and took ' \
                        'your brother with him and you never saw them again.'
            LISTOFCOMMANDS = 'North, South, East, West, Northwest, Southeast, Southwest, Up, Down, \nLook, Quit, ' \
            ...
            M_BOX = 'You wake up in a metal box. There is one path to the north. \nYou are wearing leather armor.'
            M_BOX2 = 'You are back in the metal box. Remember there is a path to the north'
            BEDROOM = 'You enter what looks like a bedroom. \nIn the upper right corner you see Sam Wilson trapped ' \
                      'in a cage, muttering something about ducks. \nIn another corner you see a rubber duck wearing ' \
                      'a fedora. \nThere is a path to the west and a path to the east.'
            ...
            KITCHEN = 'You are now in the kitchen. On the table in the upper left corner there is a pocket knife. ' \
                      '\nThere is four paths: West, North, South, and East. Also you see a very familiar person in ' \
                      'the corner.'
            KITCHEN2 = 'Welcome back to the kitchen. You see Wanda Maximoff cooking with Vision. Go either north, ' \
                       'south, east, or west'
            HOLE_1 = 'You fall down a deep hole and land on a mattress. There is a path to the south.'
            HOLE_1_2 = 'You are back in the hole. There is a path to the south'
            CAVE_1 = 'You enter a dirt cave with one torch on the floor. There is three paths to the east, north, ' \
                     'and south.'
            CAVE_1_2 = 'You re-enter a dirt cave with one torch on the floor. There is two paths to the east and ' \
            ...
            STORAGE = 'You walk into a dim lighted storage room. In the bottom right corner, \nthere is a cardboard ' \
                      'box. It seems to be partially open. There is a path to the north.'
            STORAGE2 = 'There is still a box in the corner of the room. There is a path to the north.'
            S_BOX = 'You are now in a steel box. There is a path to the north and one to the west.'
            GRASS = 'You leave the bathroom and you are now in a grassy field. To the north and to the east \nthere ' \
                    'is a brick wall. There are two paths to the west and south. \nAlso you see Tony Stark working ' \
            ...
            LIBRARY = 'You walk into a library full of dusty books. You look around and find 600 rubber ducks ' \
                      ...
                      'north.'
            LIBRARY2 = 'AHCHOO! The ducks are moving! Get out to the west, southeast, or north!'
            CLASS = 'You walk into what look likes an old classroom. \nBooks are left all over the desks and there ' \
                    'is a staircase in the back. \nThere are two paths leading out, down the staircase and to the east.'
            CLASS2 = 'Welcome to class. Please leave before Steve Rogers comes back. Head out to the east or go ' \
                     'downstairs.'
            MARSTORAGE = 'You enter a room full of seafoam green colored whiteboard markers. \nThere is two paths ' \
                         'up the staircase and down the staircase.'
            MARSTORAGE2 = 'The markers are still there. Go upstairs or downstairs.'
            LARSTORAGE = 'You enter a dim lighted storage room. You see over 3,000 tiny rubber ducks in the room ' \
                '\nsurrounding a the largest rubber duck in the world. You need to pick up the key in the room. Then ' \
                'the ducks will not attack you. You can go up the stairs and once you have the key you can go east.'
            LARSTORAGE2 = 'You are back. Not quite sure if the ducks trust you yet. Have fun I guess... :('
            PARTY = 'You gained the ducks trust. You walk into a party room to celebrate the win ' \
                '\nwith all the people who live in the house and Bruce Banner. Congrats on beating the game. You did ' \
                    'a nice job.'
            MOVIESET = 'You enter what looks like a movie set. You close your eyes and see the filming of your ' \
                       'favorite movie.'
            BASEMENT = 'Hey!!! It\'s a basement!!!! And Loki is here!!!! Probably hiding from Thor. Go up and ' \
                       'discover a secret. ' \
                       '\nLike Bucky said, he doesn\'t mind sharing his plums. Take some if you want.'
            ...
            bathroom_1 = Room('Bathroom', BATH_1, BATH_1_2, '', 'storage_room', 'grass_field', 'kitchen', '', '', '',
                              '', '', '',
                              [half_eaten_sandwich])
            hole_2 = Room('Hole', HOLE_2, HOLE_2_2, '', '', '', '', 'cave_2', '', '', '', '', '')
            cave_2 = Room('Cave', CAVE_2, CAVE_2_2, 'deadzone', '', '', '', '', 'hole_2', 'bedroom', '', '', '')
            ...
            library = Room('Library', LIBRARY, LIBRARY2, 'm_box', '', '', 'classroom', '', 'pool', '', '', '', '')
            classroom = Room('Classroom', CLASS, CLASS2, '', '', 'library', '', '', '', '', '', 'marker_storage', '')
            marker_storage = Room('Whiteboard Marker Storage', MARSTORAGE, MARSTORAGE2, '', '', '', '', '', '', '',
                                  'classroom',
                                  'large_storage', '', [sword])
            large_storage = Room('Large Storage Room', LARSTORAGE, LARSTORAGE2, '', '', 'party', '', '', '', '',
                                 'marker_storage',
                                 '', '')
            party = Room('Party Central 101', PARTY, '', '', '', '', '', '', '', '', '', '', '')
            movieset = Room('Movie Set', MOVIESET, '', '', '', '', '', '', '', '', '', '', '')
            ...
            print('I\'m sorry. I guess you will never figure out what happened to your father and brother. Bye now :)')
            is_playing = False
        else:
            is_playing = False
    ...
    if current_node == party:
        ...
        if response.lower() == 'yes':
            ...
            MOVIESET = 'You enter what looks like a movie set. You close your eyes and see the filming of your ' \
                       'favorite movie.'
            BASEMENT = 'Hey!!! It\'s a basement!!!! And Loki is here!!!! Probably hiding from Thor. Go up and ' \
                       'discover a secret. \nLike Bucky said, he doesn\'t mind sharing his plums. Take some if you ' \
                       'want.'
            COMPOUND = 'WOW! It\'s the Avengers Compound. That\'s where they came from. Go down to head back to the ' \
                       'basement. \nPepper Potts is walking around trying to figure out where everyone went.'
            DEADZONE = 'I\'m sorry. You are now dead.'
            ...
            cave_1 = Room('Cave', CAVE_1, '', 'hole', 'deadzone', 'm_box', '', '', '', '', '', '', '')
            bathroom_1 = Room('Bathroom', BATH_1, BATH_1_2, '', 'storage_room', 'grass_field', 'kitchen', '', '', '',
                              '', '', '',
                              [half_eaten_sandwich])
            hole_2 = Room('Hole', HOLE_2, '', '', '', '', '', 'cave_2', '', '', '', '', '')
            cave_2 = Room('Cave', CAVE_2, 'deadzone', '', '', '', '', '', 'hole_2', 'bedroom', '', '', '')
            storage_room = Room('Storage Room', STORAGE, STORAGE2, 'bathroom_1', '', '', '', '', '', '', '', '', '',
                                [avocado])
            s_box = Room('Box', S_BOX, '', 'kitchen', '', '', 'm_box', '', '', '', '', '', '')
            grass_field = Room('Grass Field', GRASS, GRASS2, '', 'barn', '', 'bathroom_1', '', '', '', '', '', '')
